...
let entered = false;
let row = 0;
const chosen = wordList[Math.floor(Math.random()*wordList.length)].split("");
console.log(chosen);
(function generate(){
    for(let i = 0; i < 6; i++){
        for(let j = 0; j < 5; j++){
            document.querySelector("#grid").innerHTML += `<div class="slots"></div>`
        }
    }
    const keys = ["Q","W","E","R","T","Y","U","O","I","P","A",'S','D',"F","G","H","J","K",'L',"ENTER",'Z','X','C','V','B','N','M','&#9003']
    document.querySelector("#keyboard").innerHTML += `<div id="firstRow"></div><div id="secondRow"></div><div id="thirdRow"></div>`
    for(let i = 0; i < 10; i++){
        document.querySelector("#firstRow").innerHTML += `<div id="${keys[i]}" class="letters charcs">${keys[i]}</div>`
    } for(let i = 10; i < 19; i++){
        document.querySelector("#secondRow").innerHTML += `<div id="${keys[i]}" class="letters charcs">${keys[i]}</div>`
    } for(let i = 19; i < 28; i++){
        if(i !== 19 && i !== 27){
            document.querySelector("#thirdRow").innerHTML += `<div id="${keys[i]}" class="letters charcs">${keys[i]}</div>`
        } else if(i==19) document.querySelector("#thirdRow").innerHTML += `<div class="charcs" id="enter">${keys[i]}</div>`;
